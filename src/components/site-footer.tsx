...
function FooterGroup({
  ...
  return (
    <div>
      ...
      >
        <span className="text-[10px] uppercase tracking-[0.25em] text-brand-yellow font-semibold">
          {title}
        </span>
        <ChevronDown
          className={`h-4 w-4 text-brand-yellow transition-transform md:hidden ${
            open ? "rotate-180" : ""
          }`}
        />
      </button>
      <div
        className={`${open ? "block" : "hidden"} pt-5 md:block md:pt-0`}
      >
    ...
    <footer className="bg-brand-dark text-white/80">
      <div className="max-w-7xl mx-auto px-6 pt-20 pb-12">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-10 md:gap-12 mb-16">
          ...
          <FooterGroup title="Institucional" defaultOpen>
          ...
          <FooterGroup title="Guias da região">
            ...
            <li><Link to="/guia-de-ruas" className="hover:text-brand-yellow">Guia de ruas e avenidas</Link></li>
          </FooterGroup>

          <FooterGroup title="Fale conosco" defaultOpen>
            <li>
